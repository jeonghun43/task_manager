...
- [x] **T091** `AppData.sample` — 아직 손대지 않은 첫 실행 샘플이라는 표시
- [x] **T092** `LocalStorageAdapter.normalize` — 새로고침해도 표시가 유지되도록 왕복
- [x] **T093** `useAppStore` — `isSample` 상태. `persist()` 를 지나면 곧 사용자가 건드린 것이므로
      **한 곳에서** 표시를 뗀다 (액션마다 흩어두면 언젠가 하나를 빠뜨린다).
      시드 생성은 `persist()` 를 우회해 표시를 남긴 채 저장한다.
- [x] **T094** `useSyncStore.attach` — 로컬이 샘플뿐이면 **올리지 않고 버린다**
...
- [x] **T097** `Icon.tsx` — `settings` 를 바깥 링 + 톱니로 교체(해와 구분), `user` 추가
- [x] **T098** `AccountButton.tsx` — 로그아웃 상태는 `로그인` 텍스트 버튼,
      로그인 상태는 이니셜 아바타 + 메뉴(이메일 · 동기화 상태 · 로그아웃)
- [x] **T099** 오류 상태 표시와 `다시 시도` (F3)
- [x] **T100** `Toolbar.tsx` — 설정 서랍에서 동기화 항목 제거, 계정 버튼을 오른쪽 끝에 배치
- [x] **T101** 아이콘 버튼에 `title` 부여 (F4)
