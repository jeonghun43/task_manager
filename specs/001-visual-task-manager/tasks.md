...
## Phase 17 — 팝오버가 칸에 잘리는 문제 (FR-20)

> **증상**: 과업별 뷰에서 카드의 날짜 칩이나 `⋮` 를 누르면 팝오버가 잘려 보였다.
> **원인**: 카드 안에 `absolute` 로 그렸는데 컬럼의 할 일 목록이 `overflow-y-auto` 스크롤 영역이다.

- [x] **T113** `FloatingLayer` — `document.body` 포털 + `fixed`. 아래가 좁으면 위로 뒤집고,
      좌우로 넘치면 화면 안으로 민다. 스크롤·리사이즈에 재계산(캡처 단계로 들어 안쪽 스크롤까지 잡는다)
- [x] **T114** `DueDatePicker` 를 `FloatingLayer` 로 이관 (바깥 클릭·Escape 처리도 레이어가 맡는다)
- [x] **T115** `Menu` 를 `FloatingLayer` 로 이관 — 두 곳이 각자 위치 계산을 들고 있으면 한쪽만 고쳐진 채 남는다
- [x] **T116** 5개 뷰 · 375px 회귀 확인

## Phase 18 — 삭제 되살아남 · 로그인 시 확인 (FR-21)

- [x] **T117** `MirroredAdapter` — 서버 저장이 성공하면 localStorage 에도 남긴다.
      낡은 사본이 `hydrate()` 를 통해 되살아나던 경로를 끊는다
- [x] **T118** `attach(mode)` 분리 — `restore`(서버가 진실) / `adopt`(로컬을 합침).
      병합은 삭제를 표현할 수 없으므로 **로그인하는 순간에만** 한다
- [x] **T119** 로그인 의도 표시(`sessionStorage`) — 구글에서 돌아오면 `getSession()` 이
      곧바로 세션을 주는 일이 잦아, 그것만으로는 "방금 로그인" 과 "다시 열기" 가 구분되지 않는다
- [x] **T120** 로그인 직후 확인 다이얼로그 (올리기 / 나중에)
- [x] **T121** `paused` 상태 — 계정 버튼에 `동기화 대기` + `지금 올리고 동기화 시작`
- [ ] **T122** 실제 두 기기로 AC-36~40 확인 (사용자 로그인 필요)

## Phase 19 — 로그인이 유지되지 않는 문제 (FR-22)

### 확인한 사실

**쿠키는 쓰지 않는다. 세션은 localStorage 에 있다.**
서버가 없는 정적 클라이언트 앱이라 서버 세션이라는 개념 자체가 없고, supabase-js 가
리프레시 토큰을 브라우저 저장소에 넣어 두는 방식이다.

- 저장 위치: `localStorage`, 키는 `sb-<프로젝트ref>-auth-token`
  (`supabase-js/dist/index.cjs` 의 `` `sb-${baseUrl.hostname.split(".")[0]}-auth-token` ``)
- `persistSession: true` 이므로 `globalThis.localStorage` 를 쓴다.
  localStorage 를 쓸 수 없는 환경이면 **메모리로 조용히 물러난다** — 이 경우 탭을 닫는 순간 날아간다.
- 우리 코드가 세션을 지우는 곳은 없다. `localAdapter.clear()` 는 `vtm.data.v1` 만 지운다.

즉 **설정은 정상**이고, 껐다 켜면 날아간다는 것은 아래 중 하나다.

| 후보 | 왜 그럴듯한가 | 코드로 고칠 수 있나 |
|---|---|---|
| A. 부팅 직후 네트워크가 아직 안 붙어 토큰 갱신 실패 | 노트북을 켜자마자 탭이 복원되는 상황과 정확히 겹친다 | **된다** — 복구 재시도 |
| B. 여러 탭이 동시에 복원되며 리프레시 토큰 회전 충돌 | 탭 복원은 동시에 일어난다 | 부분적으로 (재시도) |
| C. 브라우저가 종료 시 사이트 데이터 삭제 | 그 설정이 켜져 있으면 100% 이 증상 | 안 된다 — 알려주는 것까지 |
| D. 로그인한 origin 과 다시 여는 origin 이 다름 (`localhost` ↔ 배포 주소) | localStorage 는 origin 단위 | 안 된다 — 알려주는 것까지 |

A·B 는 **증상이 같다**: 토큰은 남아 있는데 세션 복원만 실패한다.
지금 코드는 그 경우 곧바로 `off` 로 떨어져 `로그인` 버튼을 보여줬다 — 사용자에게는 "로그아웃됐다" 로 보인다.

### 할 일

- [x] **T123** `authStorageKey()` · `readStoredAuth()` — 세션 토큰이 실제로 남아 있는지, 만료가 언제인지 읽는다
- [x] **T124** 시작 시 세션이 없는데 **토큰은 남아 있으면** `refreshSession()` 으로 복구를 시도한다.
      실패해도 `off` 로 떨어뜨리지 않고 복구 가능한 상태로 표시한다 (A·B)
- [x] **T125** `SIGNED_OUT` 을 무조건 로그아웃으로 보지 않는다. 저장된 토큰이 남아 있으면 일시적 실패로 처리
- [x] **T126** `online` 이벤트에 한 번 자동 재시도 — 부팅 직후 네트워크가 늦게 붙는 경우
- [x] **T127** 계정 버튼의 오류 상태를 `다시 연결` 로 바꾸고, 구글 재로그인 없이 세션 복구를 먼저 시도
- [ ] **T129** 실제로 브라우저를 껐다 켜고 · 네트워크를 끊고 확인 (AC-41·42) — 사용자 로그인 필요
- [x] **T128** `동기화 진단` — 저장 방식·키·토큰 유무·만료·origin·저장소 쓰기 가능 여부를 보여준다.
      C·D 는 코드로 고칠 수 없으므로 **어느 쪽인지 사용자가 알 수 있게** 하는 것이 최선이다
